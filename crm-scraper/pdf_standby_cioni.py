"""Gera PDF com leads do Lucas Cioni em 3 pipelines: stand_by, testando, reuniao_agendada."""
import os, certifi
...
def buscar(status_chave):
    """Retorna lista de dicts {nome, instagram, dt, tipo, conteudo} pra um status, filtrando Lucas Cioni."""
    leads = (
        sb.table('leads')
...
# Busca as 3 pipelines
pipelines = [
    ('Stand By', 'stand_by', buscar('stand_by')),
    ('Testando', 'testando', buscar('testando')),
    ('Reunião Agendada', 'reuniao_agendada', buscar('reuniao_agendada')),
]
total_geral = sum(len(p[2]) for p in pipelines)
...
class PDF(FPDF):
    def header(self):
        ...
        self.cell(0, 8, 'Lucas Cioni — Pipelines Ativas', new_x='LMARGIN', new_y='NEXT', align='L')
        self.set_font('Arial', '', 9)
        self.set_text_color(120, 120, 120)
        self.cell(0, 5, f"Stand By + Testando + Reuniao Agendada  |  Total: {total_geral} leads  |  Gerado em {datetime.now().strftime('%Y-%m-%d %H:%M')}",
...
# Cores das pipelines (matching o STATUS_HEX do CRM)
CORES = {
...
def secao(titulo, status_chave, rows):
    ...
    # Verifica se cabe mais o título + 1 linha na pagina atual; senão quebra
    if pdf.get_y() + 18 > pdf.h - pdf.b_margin:
        pdf.add_page()

    # Título da seção (banner colorido com cor da pipeline)
...
for titulo, status_chave, rows in pipelines:
    secao(titulo, status_chave, rows)

out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'standby_lucas_cioni.pdf')
pdf.output(out_path)
print(f'PDF gerado: {out_path}')
print(f'Total de leads (3 pipelines): {total_geral}')
for titulo, _, rows in pipelines:
