...
type Lead={venda?:Sale|null;chatid:string;nome:string;fone:string;status:string;responsavel:string|null;quando:string;ultimo_ts:number|null;ha:string;ultima:string;origem?:"crm";crmId?:string;hasConv?:boolean};
...
const COMMERCIAL=new Set(["CONTATAR","INTERESSADO","TESTE GRÁTIS","TESTANDO","CONVERTIDO","PERDIDO"]);
// Mesmo mapa do backend (PIPELINE_REMOTE_STATUS): etapa do painel <-> slug do Supabase.
const PANEL_SLUG:Record<string,string>={"MENSAGEM 1":"mensagem_1","MENSAGEM 2":"mensagem_2","MENSAGEM 3":"mensagem_3","STAND-BY":"stand_by","CONTATAR":"contatar","INTERESSADO":"interessado","TESTE GRÁTIS":"testando","TESTANDO":"testando_ativo","CONVERTIDO":"fechou","PERDIDO":"perdida"};
...
export default function ChatPipeline(){
 ...
 async function load(){try{
   ...
   const [crmRes,convs]=await Promise.all([
     ...
     fetch("/api/conversas").then(r=>r.ok?r.json():[]).catch(()=>[]) as Promise<Array<{chatid:string;fone:string;responsavel:string|null;ultimo_ts:number|null;quando:string;ultima:string;venda:Sale|null}>>,
   ]);
   ...
   stages.forEach((s,i)=>{
     ...
     for(const cl of crmLeads){
       ...
       cards.push({origem:"crm",crmId:cl.id,hasConv:!!conv,chatid:conv?conv.chatid:("55"+f+"@s.whatsapp.net"),
         ...
         quando:conv?conv.quando:"",ha:"",ultima:conv?conv.ultima:""});
     }
