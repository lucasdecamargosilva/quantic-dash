...
type Lead={venda?:Sale|null;chatid:string;nome:string;fone:string;status:string;responsavel:string|null;quando:string;ultimo_ts:number|null;ha:string;ultima:string};
const stages=[{label:"Esperando",value:"",color:"#7c3aed"},{label:"Sem resposta",value:"_sem_resposta",color:"#8490a3"},{label:"Mensagem 1",value:"MENSAGEM 1",color:"#3b82f6"},{label:"Mensagem 2",value:"MENSAGEM 2",color:"#6366f1"},{label:"Mensagem 3",value:"MENSAGEM 3",color:"#a855f7"},{label:"Stand-by",value:"STAND-BY",color:"#f59e0b"},{label:"Contatar",value:"CONTATAR",color:"#0ea5e9"},{label:"Interessado",value:"INTERESSADO",color:"#8b5cf6"},{label:"Teste grátis",value:"TESTE GRÁTIS",color:"#06b6d4"},{label:"Testando",value:"TESTANDO",color:"#84cc16"},{label:"Convertido",value:"CONVERTIDO",color:"#10b981"},{label:"Perdido",value:"PERDIDO",color:"#ef4444"},{label:"Removidos",value:"_ocultos",color:"#8490a3"}];
...
export default function ChatPipeline(){
 ...
 async function move(event:DragEndEvent){
  ...
  if(target==="CONVERTIDO"){openSale(source.lead,source.stage==="_ocultos");return;}
 ...
 return <div className="chat-pipeline">
  ...
  {loading?<p role="status">Carregando conversas…</p>:<DndContext sensors={sensors} onDragStart={e=>setDragged(e.active.data.current?.lead||null)} onDragCancel={()=>setDragged(null)} onDragEnd={e=>{void move(e)}}><div className="chat-pipeline-board">{stages.map((s,i)=><DropColumn stage={s.value} disabled={saving} key={s.label}><h2><i style={{background:s.color}}/>{s.label}<span>{visible[i].length}</span></h2><div className="chat-pipeline-cards">{visible[i].slice(0,limits[i]).map(l=><DragCard lead={l} stage={s.value} disabled={saving} key={l.chatid} onOpen={()=>{setClosing(false);setSelected({lead:l,removed:s.value==="_ocultos"})}}><strong>{l.nome||l.fone}</strong><small>{l.fone}</small><span className="chat-pipeline-owner">♙ {l.responsavel||"Sem responsável"}</span>{l.venda&&<span className="pipeline-sale-badge">{l.venda.plano} · {(l.venda.valor_centavos/100).toLocaleString("pt-BR",{style:"currency",currency:"BRL"})}/mês</span>}<p>{l.ultima||"Sem mensagem disponível"}</p><footer><span>{l.status}</span><time>{l.quando}</time></footer></DragCard>)}{!visible[i].length&&<p className="chat-pipeline-empty">Nenhuma conversa</p>}{visible[i].length>limits[i]&&<button onClick={()=>setLimits(v=>v.map((n,j)=>i===j?n+60:n))}>Mostrar mais</button>}</div></DropColumn>)}</div><DragOverlay>{dragged&&<div className="chat-pipeline-card chat-pipeline-drag-preview"><strong>{dragged.nome||dragged.fone}</strong><small>{dragged.responsavel||"Sem responsável"}</small></div>}</DragOverlay></DndContext>}
